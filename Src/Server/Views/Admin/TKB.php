...
?>
<h1 class="text-center p-4 fw-bold">Thời Khóa Biểu</h1>

<div class="d-flex gap-2 p-2 justify-content-end">

    <a class="btn btn-primary" href="/Admin/EditTkb">Thêm thời khóa biểu</a>
                    ...
                    <section class="m-auto bg-white p-4 rounded-4 text-dark">
                        <table class="table table-striped">
                            ...
                            foreach ($data["data"] as $index => $datum) {
                                ...
                                $delete = "";
                                $edit = <<<wut
                                    <a href="/Admin/EditTkb/{$datum["ID_DSMH"]}/{$datum["TIET_BAT_DAU"]}/{$datum["LOP"]}/{$datum["HK"]}/{$datum["NAM"]}" class="btn btn-primary">Chỉnh</a>
                                wut;
                                if ($datum["CAN_DELETE"] == 1) {
                                    $delete = <<<wut
                                    <a href="/Admin/DeleteTkb/{$datum["ID_DSMH"]}/{$datum["TIET_BAT_DAU"]}/{$datum["LOP"]}/{$datum["HK"]}/{$datum["NAM"]}" class="btn btn-danger">Xóa</a>
                                wut;
                                }
                                echo <<<JIT
                            <tr>
                                <th scope="row">{$datum["ID_DSMH"]}</th>
